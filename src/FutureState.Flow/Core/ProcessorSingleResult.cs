...
namespace FutureState.Flow.Core
{
        ...
        where TEntityOut : class, new()
    {
        ...
        protected IEnumerable<TEntityIn> Read()
        {
            return _reader();
        }
        ...
        public ProcessResult Process(ProcessorHandler<TEntityIn> processHandler)
        {
            ...
            ProcessorHandler = processHandler;

            ProcessedItems = new List<TEntityOut>(); // clear

            var result = processHandler.Process();

            ProcessedErrors = processHandler.Errors;

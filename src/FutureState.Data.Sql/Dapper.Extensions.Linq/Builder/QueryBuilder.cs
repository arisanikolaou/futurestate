...
namespace Dapper.Extensions.Linq.Builder
{
    public class QueryBuilder<T> : IQueryBuilder<T> where T : class
    {
        ...
        public IPredicateGroup GetPredicate()
        {
            return _predicateGroup;
        }

        private IQueryBuilder<T> Find<TValue>(Expression<Func<T, TValue>> expression, object value, Operator @operator,
            bool not)
        {
            ...
            _predicateGroup.Predicates.Add(new FieldPredicate<T>
            {
                Not = not,
                Operator = @operator,
        ...
        private sealed class PredicateConverter
        {
            ...
            private IPredicate ParseField(BinaryExpression expression)
            {
                ...
                if (TryGetField(expression.Left, out propertyName))
                    ...
                    throw new NotImplementedException("The expression is not supported.");
            ...
            private IPredicate ParseUnaryNot(UnaryExpression expression)
            {
                ...
                var predicate = Parse(expression.Operand);

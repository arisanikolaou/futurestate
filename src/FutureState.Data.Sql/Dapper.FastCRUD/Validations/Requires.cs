...
namespace Dapper.FastCrud.Validations
{
    ...
    internal static class Requires
    {
        ...
        public static T NotDefault<T>(T value, string parameterName)
        {
            if (EqualityComparer<T>.Default.Equals(value, default(T)))
                throw new ArgumentOutOfRangeException(parameterName,
                    $"Parameter {parameterName} is expected to have a value different than its default.");
        ...
        public static void NotNullOrWhiteSpace(string value, string parameterName)
        {
            ...
            if (string.IsNullOrWhiteSpace(value))
                throw new ArgumentException(
                    $"Parameter '{parameterName}' is either empty or contains only whitespaces.", parameterName);
        }
        ...
        public static void NotNullOrEmpty(IEnumerable values, string parameterName)
        {
            ...
            if (!hasElements)
                throw new ArgumentException($"Encountered an empty collection for parameter '{parameterName}'",
                    parameterName);
        }

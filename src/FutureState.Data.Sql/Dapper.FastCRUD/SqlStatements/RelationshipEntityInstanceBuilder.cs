    ...
    internal class RelationshipEntityInstanceBuilder
    {
        ...
        public RelationshipEntityInstanceIdentity<TEntity> RegisterResultSetRowInstance<TEntity>(TEntity entity)
        {
            ...
            if (!_entityInstanceContainers.TryGetValue(typeof(TEntity), out instanceContainer))
                throw new InvalidOperationException(
                    $"Type '{typeof(TEntity)}' could not be found in the list of registered instance containers.");

            var instanceIdentity = new RelationshipEntityInstanceIdentity<TEntity>(instanceContainer.EntityMapping,
                entity);
            if (entity == null)
            {
        ...
        public void EndResultSetRow()
        {
            if (_currentRowParticipatingEntityCount != _currentRowEntityInstances.Length)
                throw new InvalidOperationException(
                    "Invalid number of registered entity instances detected for a particular row in the result set");

            for (var rowMainEntityIndex = 0;
                rowMainEntityIndex < _currentRowParticipatingEntityCount;
                rowMainEntityIndex++)
            {
                var rowMainEntityInstance = _currentRowEntityInstances[rowMainEntityIndex];

                for (var rowChildEntityIndex = rowMainEntityIndex + 1;
                    rowChildEntityIndex < _currentRowParticipatingEntityCount;
                    rowChildEntityIndex++)
                {
                    ...
                    {
                        Bind(rowMainEntityInstance.EntityMapping, rowMainEntityInstance.UniqueInstance,
                            rowChildEntityInstance.EntityMapping, rowChildEntityInstance.UniqueInstance);
                        Bind(rowChildEntityInstance.EntityMapping, rowChildEntityInstance.UniqueInstance,
                            rowMainEntityInstance.EntityMapping, rowMainEntityInstance.UniqueInstance);
                    }
                }
            }
        ...
        private static void Bind(EntityMapping mainEntityMapping, object mainEntity, EntityMapping childEntityMapping,
            object childEntity)
        {
        ...
        private class EntityInstanceContainer
        {
            ...
            public EntityInstanceContainer(EntityMapping entityMapping)
            {
                ...
                KnownInstances = new Dictionary<RelationshipEntityInstanceIdentity, object>();
            }

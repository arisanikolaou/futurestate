...
namespace Dapper.FastCrud.Configuration
{
    ...
    public class OrmConventions
    {
        ...
        private static readonly HashSet<Type> _simpleSqlTypes = new HashSet<Type>
        {
            typeof(byte),
            typeof(sbyte),
            typeof(short),
        ...
        protected internal OrmConventions()
        {
            ...
            AddEntityToTableNameConversionRule("$", "s");
            AddEntityToTableNameConversionRule("s$", "s");
            AddEntityToTableNameConversionRule("(ax|test)is$", "$1es");
            AddEntityToTableNameConversionRule("(octop|vir|alumn|fung)us$", "$1i");
            AddEntityToTableNameConversionRule("(alias|status)$", "$1es");
            AddEntityToTableNameConversionRule("(bu)s$", "$1ses");
            AddEntityToTableNameConversionRule("(buffal|tomat|volcan)o$", "$1oes");
            AddEntityToTableNameConversionRule("([ti])um$", "$1a");
            AddEntityToTableNameConversionRule("sis$", "ses");
            AddEntityToTableNameConversionRule("(?:([^f])fe|([lr])f)$", "$1$2ves");
            AddEntityToTableNameConversionRule("(hive)$", "$1s");
            AddEntityToTableNameConversionRule("([^aeiouy]|qu)y$", "$1ies");
            AddEntityToTableNameConversionRule("(x|ch|ss|sh)$", "$1es");
            AddEntityToTableNameConversionRule("(matr|vert|ind)ix|ex$", "$1ices");
            AddEntityToTableNameConversionRule("([m|l])ouse$", "$1ice");
            AddEntityToTableNameConversionRule("^(ox)$", "$1en");
            AddEntityToTableNameConversionRule("(quiz)$", "$1zes");
            AddEntityToTableNameConversionRule("(campus)$", "$1es");
            AddEntityToTableNameConversionRule("^is$", "are");
        }
        ...
        ///     In order for the schema to be used, you must also ensure that <see cref="IsUsingSqlSchemas" /> for the dialect also
        ///     returns <c>true</c>.
        /// </summary>
        public virtual string GetSchemaName(Type entityType)
        {
            return GetEntityAttributes(entityType).OfType<TableAttribute>().FirstOrDefault()?.Schema;
        }
        ...
        public virtual void ConfigureEntityPropertyMapping(PropertyMapping propertyMapping)
        {
            ...
            if (string.Equals(propertyMapping.PropertyName, "id", StringComparison.OrdinalIgnoreCase)
                && GetEntityAttributes(propertyMapping.EntityMapping.EntityType).Length == 0
                && !GetEntityProperties(propertyMapping.EntityMapping.EntityType).Any(
                    propDesc => GetEntityPropertyAttributes(propertyMapping.EntityMapping.EntityType, propDesc).Any(
                        propAttr =>
                            propAttr is ColumnAttribute || propAttr is KeyAttribute ||
                            propAttr is DatabaseGeneratedAttribute)))
            {
            ...
            var propertyAttributes = GetEntityPropertyAttributes(propertyMapping.EntityMapping.EntityType,
            ...
            if (
                databaseGeneratedAttributes.Any(
                    dbGenerated => dbGenerated.DatabaseGeneratedOption == DatabaseGeneratedOption.Identity))
                propertyMapping.SetDatabaseGenerated(DatabaseGeneratedOption.Identity);

            if (
                databaseGeneratedAttributes.Any(
                    dbGenerated => dbGenerated.DatabaseGeneratedOption == DatabaseGeneratedOption.Computed))
            ...
            {
                var referencingTypePropertyDescriptor = TypeDescriptor.GetProperties(
                        propertyMapping.Descriptor.ComponentType)
                    .OfType<PropertyDescriptor>()
                    .Single(propDescriptor => propDescriptor.Name == foreignKeyAttribute.Name);
                propertyMapping.SetChildParentRelationship(referencingTypePropertyDescriptor.PropertyType,
                    referencingTypePropertyDescriptor.Name);
            }
        }
        ...
        private Attribute[] GetEntityPropertyAttributes(Type entityType, PropertyDescriptor property)
        {
            ...
            var entityMetadataAttribute =
                TypeDescriptor.GetAttributes(entityType).OfType<MetadataTypeAttribute>().FirstOrDefault();
            var entityMetadataProperty = entityMetadataAttribute == null
                ? null
                : TypeDescriptor
                    .GetProperties(entityMetadataAttribute.MetadataClassType)
                    .OfType<PropertyDescriptor>()
                    .FirstOrDefault(propDescriptor => propDescriptor.Name == property.Name);
            var entityMetadataPropertyAttributes = entityMetadataProperty?.Attributes.OfType<Attribute>() ??
                                                   new Attribute[0];
